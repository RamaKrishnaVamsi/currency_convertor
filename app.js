...
// Create country_list object from country_code (assuming flag codes match currency codes)
let country_list = {};
for (let currency in country_code) {
    country_list[currency] = country_code[currency];
}

let apiKey = "fca_live_1o2bTQMgx6beDXLmlHjZ9Gp1S9wygNs5fL5Ldia9";
...
const getExchangeRate= () => {
    ...
    let url = `https://api.freecurrencyapi.com/v1/latest?apikey=fca_live_1o2bTQMgx6beDXLmlHjZ9Gp1S9wygNs5fL5Ldia9&currencies=&base_currency=INR`
   fetch(url)
        .then((response) => {
            if (!response.ok) {
        ...
        .then((result) => {
            if (result.data && result.data[toCurrency.value]) {
                ...
                exchangeRateText.innerText = "Currency data not available";
            }
        })
        .catch((error) => {
